...
class Submission(SubmissionSpec12):
    ...
    def _estimate_transition_probabilites(self, annotated_sentences):
        ...
        for tag in tag_set:
            estimate_transition_probabilites[tag] = defaultdict(
                lambda: delta / (tags_frequency[tag] + delta * len_tags))

        for (prev_tag, tags) in tags_pair_frequency.items():
            for (tag, count) in tags.items():
                estimate_transition_probabilites[prev_tag][tag] = (count + delta) / (
                        tags_frequency[prev_tag] + delta * len_tags)
    ...
    def _viterbi(self, observations, state_graph):
        ...
        for state in state_graph:
            viterbi[state][0] = estimate_transition_probabilites['<s>'][state] * \
        ...
        for time_step in range(1, len_observations):
            ...
            for state in state_graph:
                for _state in state_graph:
                    _dict[_state] = viterbi[_state][time_step - 1] * estimate_transition_probabilites[_state][state] * \
